...
export function BusinessMessagesManager({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const res = await fetch("/api/business/chat/conversations");
  ...
  if (conversations.length === 0) {
    return (
      <p className="text-sm text-ink-400">
        {locale === "vi" ? "Chưa có tin nhắn nào từ khách hàng." : "No messages from customers yet."}
      </p>
    );
  }
          ...
          >
            ...
            {c.lastMessage && (
              <p className="mt-0.5 truncate text-xs text-ink-400">
                {c.lastMessage.senderType === "BUSINESS" ? (locale === "vi" ? "Bạn: " : "You: ") : ""}
